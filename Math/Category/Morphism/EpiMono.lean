...
## 定理
### `Category.hom.IsMono` / `IsEpi`
- `.cancel` — cancellation law（`f ○ h = f ○ k ↔ h = k`）
- `.comp` — composition 繼承（mono ○ mono = mono 等）
- `.mono_of_mono` / `.epi_of_epi` — 從合成推出
### `Category.hom.IsSplitMono` / `IsSplitEpi`
- `.IsMono` / `.IsEpi` — split ⟹ 一般
-/
